...
> Renamed from `security-review` so it no longer overrides Claude Code's built-in `/security-review` command. Use both: `/security-review` scans the current changes for vulnerabilities; this skill carries LisanHub's own security rules.
